...
class subscribeChangelog(object):
		...
		if type == 'Lustre':
			...
			t1 = threading.Thread(target=self.receiveCeph)
                        t1.setDaemon(True)
                        t1.start()
                        t1.join()
		...
		check = 0 
                while True:
                        ...
                        if msg:
                                msg = msg.split(",")
                                if (path == msg[2][:len(path)]):
                                        if msg[3] == "UPDATE:(openc)":
                                                print(path + ' CREATE ' + msg[2][len(path):])

                                        elif msg[3] == "UPDATE:(mkdir)":
                                                print(path + ' CREATE,ISDIR ' + msg[2][len(path):])

                                        elif msg[3] == "UPDATE:(unlink_local)":
                                                print(path + ' DELETE ' + msg[2][len(path):])

                                        #elif msg[3] == "07RMDIR":
                                                #print(path+' DELETE,ISDIR '+msg[2][len(path):])

                                        elif msg[3] == "UPDATE:(rename)": 
                                                print(path +' MOVED ' + msg[2][len(path):])

                                        #elif msg[3] == "11CLOSE":
                                                #print(path+' CLOSE '+msg[2][len(path):])

                                        elif msg[3] == "UPDATE:(cap_update)":
                                                print(path+' MODIFY '+msg[2][len(path):])
						print(path+' CLOSE '+msg[2][len(path):])
...
if __name__ == '__main__':
	global config
   	config = ConfigParser.ConfigParser()
    	config.read('config_ceph_client.ini')
